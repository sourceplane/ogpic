...
The verification suite for **usage counters and quota evaluation**. Runs in CI as a `quick-check` lane on
every plan (`pnpm --filter ./tests/metering-worker test` locally) against in-memory
