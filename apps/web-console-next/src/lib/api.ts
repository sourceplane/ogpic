...
// deleted; all wire I/O now goes through the typed `Ogpic` resource
...
//   - `createClient` — constructs a `Ogpic` against a target, wired
...
//     `Promise<T>` + thrown `OgpicError` into the same shape the
...
import { Ogpic, OgpicError, type ClientOptions } from "@saas/sdk";
import { apiEdgeWorkersDevUrl } from "./app-config";

export type ApiClient = Ogpic;
...
export function createClient(target: ApiTarget, token: string | null): Ogpic {
  const opts: ClientOptions = { baseUrl: target.url };
  if (token) opts.auth = { kind: "bearer", token };
  return new Ogpic(opts);
}

/**
 * Adapt an SDK promise into the `ApiResult<T>` envelope. Typed
 * `OgpicError` subclasses surface as `{ ok: false }` carrying the
