 ...
 * the live useRondo view-model: Home (roster pitch), Rate (segment scales via the
 * voting flow), Games (fixtures + results). No scheduling, no squad admin.
 *
...
export function PlayerApp({ vm }: { vm: RondoVM }) {
  const [view, setView] = React.useState<View>("pitch");

  // Rate flow: drive the VM's vote target through the unrated queue.
  const ratedSet = new Set(vm.rated);
  const queue = vm.players.filter((p) => !ratedSet.has(p.id));
  React.useEffect(() => {
    if (view === "rate" && !vm.voteTargetP && queue[0]) vm.setVoteTarget(queue[0].id);
  }, [view, vm.voteTargetP, queue.length]);

  const nav = <BottomNavPlayer active={view as PlayerTab} rateBadge={queue.length} onSelect={(t) => setView(t)} />;
    ...
    return (
      <PhoneShell>
        ...
        <div style={{ padding: "12px 24px 0", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <span style={{ fontFamily: MONO, fontSize: 11, fontWeight: 600, color: C.green }}>{String(vm.ratedCount).padStart(2, "0")} / {vm.totalRatable}</span>
        </div>
        ...
        {t ? (
          <div style={{ flex: 1, margin: "16px 20px 24px", background: C.card, border: `1px solid ${ink(0.1)}`, borderRadius: 22, padding: "26px 22px", display: "flex", flexDirection: "column", minHeight: 0 }}>
            ...
            <div style={{ flex: 1 }} />
            <div onClick={() => vm.submitVote()} className="rk-press" style={{ height: 54, borderRadius: 16, background: C.ink, color: C.onDark, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, fontWeight: 700 }}>Submit &amp; next →</div>
          </div>
        ) : (
