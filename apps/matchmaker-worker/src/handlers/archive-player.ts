...
export interface HandleArchivePlayerDeps {
  repo?: MatchmakerRepository;
}

export async function handleArchivePlayer(
  env: Env,
  requestId: string,
  actor: ActorContext,
  orgId: Uuid,
  playerId: Uuid,
  deps?: HandleArchivePlayerDeps,
): Promise<Response> {
  ...
  try {
    ...
    const result = await repo.archivePlayer(orgId, playerId, new Date());
    if (!result.ok) {
