...
/** A decoded keyset cursor. `t` is the ISO sort-key value (createdAt or
 *  scheduledAt depending on the list), `id` the UUID tiebreaker. */
...
export function encodeCursor(t: string, id: string): string {
  const payload = JSON.stringify({ v: CURSOR_VERSION, t, i: id });
  return btoa(payload);
}
