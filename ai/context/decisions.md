...
- AWS IAM roles and shared Terraform state buckets are owned by `aws-admin`.
  `ogpic` consumes the repo-scoped roles created there.
- Terraform state for `ogpic` uses the shared S3 buckets
...
- Supabase target is fixed:
  ...
  - project/database names `ogpic-stage` and
    `ogpic-prod`
...
- BF5 (saas-bootstrap-factory): infra components publish their consumable
  ...
  write scope (`sourceplane/ogpic/*`) already cover it.
- BF4 (saas-bootstrap-factory): the Hyperdrive binding name is `PLATFORM_DB`
  (was `OGPIC_DB`; epic decision D1). Renamed atomically across all 12
...
- Task 0040 CORS policy: `api-edge` allows only the Pages production origin
  (`https://ogpic-web-console.pages.dev`), Pages preview subdomains
  (`*.ogpic-web-console.pages.dev`), and localhost/127.0.0.1 for dev.
...
- Web-console hosting should be environment-specific after Task 0040: stage and
  ...
  names for Task 0041 are `ogpic-web-console-stage` and
  `ogpic-web-console-prod`; each deployed console should be locked to its
...
- Verified plan role ARNs from Task 0004:
  - `arn:aws:iam::306024784101:role/dev-github-sourceplane-ogpic-plan`
  - `arn:aws:iam::306024784101:role/stage-github-sourceplane-ogpic-plan`
  - `arn:aws:iam::306024784101:role/prod-github-sourceplane-ogpic-plan`
- Verified deploy role ARNs from Task 0004:
  - `arn:aws:iam::306024784101:role/dev-github-sourceplane-ogpic-production-deploy`
  - `arn:aws:iam::306024784101:role/stage-github-sourceplane-ogpic-production-deploy`
  - `arn:aws:iam::306024784101:role/prod-github-sourceplane-ogpic-production-deploy`
- `aws-admin` PR #26 and main CI run `26204953054` verified Secrets Manager
  lifecycle access for `sourceplane/ogpic/*`.
- `ogpic` main CI run `26209010693` verified Supabase stage/prod
