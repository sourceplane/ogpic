...
| [`saas-orun-secrets/`](./saas-orun-secrets/) | **OS** | Draft (OS0 shipped) | tests/orun-secrets (new), stack-tectonic worker compositions, all wrangler templates, tooling/secrets-sync (retired at OS6), specs/core/access-and-infra.md | Migrate every ogpic secret from AWS Secrets Manager to **orun secrets** (`secret://…` refs; deploy-time materialization + run-time injection), sequenced Cloudflare → Supabase → integrations, then retire `secrets-sync`. The adopter half of `orun`'s `orun-secrets` (SEC) + `orun-cloud`'s `saas-secret-manager` (SM); supersedes `saas-secrets-sync`. |
...
| [`catalog-docs-adoption/`](./catalog-docs-adoption/) | **CD** (adopter half of orun-cloud `saas-catalog-docs`) | Draft — waiting on the orun provider bump (CD1/CD2) | `intent.yaml`, `docs/*`, per-worker `component.yaml` | ogpic as the **reference adopter** of the catalog doc-set model: pre-authored `docs/{architecture,runbook}.md` now; declares `repo.docs.pages` + one `domain` enrichment when the pinned orun carries `saas-catalog-docs` CD1/CD2 — making this workspace the worked example the cloud's Docs hub demos. |
| [`matchmaker/`](./matchmaker/) | **MM** | In progress | new matchmaker-worker, api-edge facade, packages/{contracts,policy-engine,db,sdk,cli}, web-console-next | The starter's first **product** bounded context, repurposed from the "MatchMaker 26" draft-engine HTML: an org-scoped shared roster, a deterministic server-side balancing draft, and fixtures with result recording + a server-generated share payload. Backend (MM1–MM3) built end to end; console (MM4), public share link (MM5), audit (MM6), and bulk import/export (MM7) follow. |
| [`rondo-experience/`](./rondo-experience/) | **RX** (extends **U** + **MM**) | Draft | web-console-next, packages/ui, matchmaker-worker, api-edge, packages/{contracts,db,sdk,cli,policy-engine} | The full **UI revamp** to *Rondo* — a mobile-first, pixel-matched football-community app — plus the social loop it implies: RX0 design system + responsive app shell · RX1 auth/onboarding · RX2 squad home · RX3 voting→OVR · RX4 availability+draft · RX5 live match · RX6 fixtures · RX7 members · RX8 community/leaderboard · RX9 desktop/theming/a11y · RX10 verification. Supersedes MM4's desktop pages; adds additive voting/availability/live-scoring/points backend slices. |
