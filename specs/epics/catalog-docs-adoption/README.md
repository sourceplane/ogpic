...
| Pipeline note | The first main convergence after the merge (run `28734466789`) surfaced four latent platform bugs at this repo's 98-job scale, all fixed in orun-cloud the same day: an unseeded coordination shard on replayed createRun (#346), a partial `run_jobs` read model (#348), per-row DAG seeding slow enough to time clients out (#350 — now one statement), and log appends fenced on an eventually-consistent lease projection (#351). The **catalog autopush was also rate-limited into a silent skip** — the CAS object plane now rides its own edge rate-limit family (#352) — so the doc-set catalog lands with the first main push after that deploy (this one). |
