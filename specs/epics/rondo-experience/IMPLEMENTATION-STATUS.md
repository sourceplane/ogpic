...
| RX7 | **UI built + captain backend shipped** | Manage squad UI (invite code, pending requests, members & roles). The **captain** designation is now a real backend slice (`220_matchmaker_captain` migration adds `is_captain` with an at-most-one-per-org partial unique index; `PUT /players/:id/captain` → SDK `client.roster.setCaptain` → CLI `matchmaker player captain`, `organization.roster.write` RBAC + handler tests). Membership-role + add-player + set-captain UI wiring pending |
