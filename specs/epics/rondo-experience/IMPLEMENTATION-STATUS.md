...
| RX6 | **Scheduling live (venue)** | The Fixtures "Schedule a match" card now creates a real fixture: it auto-drafts the available squad into two sides (vote-blended ratings) and persists them at the selected turf as the match **venue**. Backend slice `250_matchmaker_match_venue` (migration `venue_name`/`venue_address`/`venue_booked` + repo + contracts `MatchVenue` + create/update-match parsing + mappers + SDK types). Recent-results rows show the venue. Availability-request emails + RSVP + pre-match reminders (Feature 6 b/c) remain the next increments |
