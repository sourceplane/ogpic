...
| RX6 | **Scheduling + availability-request emails** | The Fixtures "Schedule a match" card creates a real fixture (auto-draft → persist at the selected turf as the **venue**; slice `250_matchmaker_match_venue`). On schedule, the matchmaker worker now **emails every player with a contact address** a `match.availability_request` (best-effort, post-commit, per-(match,player) idempotent, skipped under `DEBUG_DELIVERY`) via a new `NOTIFICATIONS_WORKER` binding + notifications template. Players set availability (in/maybe/out) by signing in — an **unauthenticated RSVP ingress** (RSVP straight from the email link) and **pre-match cron reminders** (Feature 6c) remain the final increments |
| RX7 | **Captain live + add-player** | Manage squad UI wired to live roster: **set captain** (`client.roster.setCaptain`, exactly one per org) and **release** (`client.roster.release`) with optimistic updates; the captain is surfaced on the Squad card and as a Ⓒ badge on the FUT card. Backend slice `220_matchmaker_captain`. **Add a player** now works from Manage squad (name + position + optional RSVP email); attributes may be omitted so the roster seeds a **default OVR-60 strength** (Feature 5's "a default strength score will be added"), adjustable by the manager or shifted by voting. Backend slice `260_matchmaker_player_email` (nullable `email` on players + optional-attributes create + validators). Org-member role changes (`client.memberships.updateMemberRole`) remain a follow-up |
| RX7b | **Join-by-code live** | Self-serve membership: a manager shares the squad's **join code** (`client.memberships.getJoinCode`, lazily minted, rotatable), a signed-in player requests to join at `/rondo/join` (`client.memberships.join({code})`, cross-org), and the manager approves/declines from *Manage squad* (`approveJoinRequest`/`declineJoinRequest`, approval mints member + role assignment). Backend slice `230_membership_join_requests` (migration + policy `organization.join_code.*` / `organization.join_request.*` → owner+admin + handlers + edge + SDK + CLI + tests) |
