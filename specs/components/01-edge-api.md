...
|----------------|-------------------------|
| stage | `https://${CONSOLE_CUSTOM_DOMAIN}` (`https://stage.ogpic.app`), `https://ogpic-web-console-next-stage.<workers-dev-subdomain>.workers.dev`, localhost |
| prod | `https://${CONSOLE_CUSTOM_DOMAIN}` (`https://prod.ogpic.app`), `https://ogpic-web-console-next-prod.<workers-dev-subdomain>.workers.dev`, localhost |
...
Legacy `ogpic-web-console-{env}.pages.dev` Pages origins were removed
...
`prod.ogpic.app` calling stage API) are rejected. Custom domain origins
