...
|----------------|-------------------------|
| stage | `https://${CONSOLE_CUSTOM_DOMAIN}` (`https://stage.lumen.app`), `https://lumen-web-console-next-stage.<workers-dev-subdomain>.workers.dev`, localhost |
| prod | `https://${CONSOLE_CUSTOM_DOMAIN}` (`https://prod.lumen.app`), `https://lumen-web-console-next-prod.<workers-dev-subdomain>.workers.dev`, localhost |
...
Legacy `lumen-web-console-{env}.pages.dev` Pages origins were removed
...
`prod.lumen.app` calling stage API) are rejected. Custom domain origins
