...
V1 must ship with at least one first-party sign-in path that is Lumen-owned and served through the Worker runtime, such as email magic link or one-time code. Additional OAuth providers may be added through adapters, but hosted auth SaaS, including Supabase Auth, is not the starter source of truth unless a future spec explicitly changes that boundary.
