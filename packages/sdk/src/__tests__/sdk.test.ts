...
describe("Transport — success path", () => {
  it("unwraps the data envelope on GET", async () => {
    ...
    const client = new Lumen({ baseUrl: "https://api.test", fetch });
...
describe("Error decoding — typed branches", () => {
  ...
  const cases: Array<[string, number, new (...args: never[]) => LumenError]> = [
  ...
  it("forward-compat: unknown error code → base LumenError", async () => {
    ...
    try {
      ...
      expect((err as LumenError).code).toBe("future_quota_exceeded");
    }
  });
