...
describe("Transport — success path", () => {
  it("unwraps the data envelope on GET", async () => {
    ...
    const client = new Ogpic({ baseUrl: "https://api.test", fetch });
...
describe("Error decoding — typed branches", () => {
  ...
  const cases: Array<[string, number, new (...args: never[]) => OgpicError]> = [
  ...
  it("forward-compat: unknown error code → base OgpicError", async () => {
    ...
    try {
      ...
      expect(err).toBeInstanceOf(OgpicError);
      expect(err).not.toBeInstanceOf(BadRequestError);
      expect((err as OgpicError).code).toBe("future_quota_exceeded");
    }
  });
